...
class User(db.Model):
    ...
    telegram_2fa_enabled = db.Column(db.Boolean, default=False)
    telegram_link_code = db.Column(db.String(20))
    telegram_link_expires = db.Column(db.DateTime)
...
class UserActivity(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    activity_type = db.Column(db.String(50), default='listening')  # listening, gift_received
    track_id = db.Column(db.String(100))
    track_title = db.Column(db.String(200))
    track_artist = db.Column(db.String(200))
    track_cover = db.Column(db.String(500))
...
class Playlist(db.Model):
    ...
    share_code = db.Column(db.String(20))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    tracks = db.relationship('PlaylistTrack', backref='playlist', lazy='dynamic', cascade='all, delete-orphan')
    collaborators = db.relationship('PlaylistCollaborator', backref='playlist', lazy='dynamic', cascade='all, delete-orphan')
