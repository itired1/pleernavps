...
def get_artist_names(artists):
    """Safely extract artist names from Yandex Music track"""
    if not artists:
        return []
    result = []
    for a in artists:
...
def get_vk_api(token):
    ...
    try:
        ...
        def vk_call(method, values=None, **kwargs):
            return original_method(method, values or {})
        vk.call = vk_call
        return vk
    except Exception as e:
                        ...
                        if chart and chart.chart.tracks:
                            for track in chart.chart.tracks[:10]:
                                ...
                                recommendations.append({
                                    ...
                                    'title': t.title,
                                    'type': 'track',
                                    'artists': get_artist_names(t.artists),
                    ...
                    if liked_service == 'vk' and liked_tracks:
                        ...
                        if track_ids:
                            for track_id in track_ids:
                                try:
                                    audio = vk.call('audio.getById', {'audios': f"-{track_id}"})
                                    if audio:
                                        t = audio[0]
                                        recommendations.append({
                                            'id': f"vk_{t['id']}",
                        ...
                        recs = vk.call('audio.getRecommendations', {'count': 10})
                        if 'items' in recs:
